...
async def post_article(title: str, excerpt: str, content: str, author: str, read_time: str, category: str, image_url: str, email, password, published: bool = True):
    """
    ...
    """
    try:
        supabase: Client = Client(SUPABASE_URL, SUPABASE_ANON_KEY)

        # Authenticate user with GitHub
        auth_response = await supabase.auth.signInWithOAuth({ 'provider': 'github' })

        print(f"Auth response: {auth_response}")

        if auth_response and auth_response.error:
        ...
        response = await supabase.table("blog_posts").insert(data).execute()
...
if __name__ == "__main__":
    ...
    **Key Benefits:**
    *   **Reduced Electricity Bills:** Generate your own power and significantly lower or eliminate your monthly electricity costs.
    ...
    **Getting Started:**
    ...
    async def main():
        ...
        if auth_response and auth_response.error:
            print(f"Authentication failed. Error: {auth_response.error}")
            exit()
        ...
        try:
            ...
            if response.data:
                print(f"Successfully posted article: '{article_title}' with slug: '{article_slug}'")
                print("Response data:", response.data)
            else:
                print(f"Failed to post article: '{title}'")
            print("Error:", response.error)
